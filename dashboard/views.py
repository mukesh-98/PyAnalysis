...
def dashboard(request):
    if request.GET['user']=='AllUser' and request.GET['app']=='AllApp':
        appname=request.GET['app']
        SomeStartdate=pd.to_datetime(request.GET['startdate']).date()
        SomeEnddate=pd.to_datetime(request.GET['enddate']).date()
    ...
    elif request.GET['user']=='AllUser':
        data1=pd.read_csv("./static/dataframe.csv")
        SomeAppname=request.GET['app']
        SomeUsername=request.GET['user']
        SomeStartdate=request.GET['startdate']
        SomeEnddate=request.GET['enddate']
        modelcount=len(data1['model_name'].unique().tolist())
        stepcount=len(data1['stepname'].unique().tolist())
        usercount=len(data1['username'].unique().tolist())
        avginc=sum(data1['Inclination'])/len(data1['Inclination'])
